...
// Base function to send an email with the configured transporter
async function sendMailTemplate(to: string, subject: string, html: string) {
...
// Generate the items HTML table body
...
export async function sendOrderEmails(orderData: any, customerEmail: string | null, shouldSendToCustomer: boolean) {
    try {
        const itemsHtml = generateItemsHtml(orderData.items);

        // Admin Email Template
        const adminSubject = `[ĐƠN HÀNG MỚI] - ${orderData.orderId}`;
        const adminHtml = `
        ...
        // Send to Admin (sent only if it's a COD order)
        if (orderData.paymentMethod === 'cod') {
            await sendMailTemplate(process.env.SMTP_EMAIL as string, adminSubject, adminHtml);
        }

        // Customer Email Template (sent only if valid email and requested AND it's a COD order)
        if (shouldSendToCustomer && customerEmail && orderData.paymentMethod === 'cod') {
            const customerSubject = `Xác nhận đơn hàng #${orderData.orderId} từ TLECTRIC`;
            const customerHtml = `
            ...
            await sendMailTemplate(customerEmail, customerSubject, customerHtml);
        }
